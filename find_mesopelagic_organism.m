function [mo, mo_fdom, beta_spike, fchl_spike, fdom_spike] = find_mesopelagic_organism(p, beta, fchl, fdom)
...
if nargin > 3; if size(p) ~= size(fdom); error('Input vectors are different size.'); end; end
...
if size(p,1) < 5
  ...
  beta_spike = false(size(beta));
  fchl_spike = false(size(fchl));
  if nargin > 3; mo_fdom = mo; fdom_spike = false(size(fdom)); end                
  return
end
...
beta_features = compute_features(beta_cluster, p, beta);
beta_features_qc = qc_features(beta_features);
fchl_spike = get_spikes(fchl, p);
% fchl_cluster = cluster_spikes(p, fchl_spike);
% fchl_features = compute_features(fchl_cluster, p, fchl);
% fchl_features_qc = qc_features(fchl_features);
...
fdom_features = compute_features(fdom_cluster, p, fdom);
fdom_features_qc = qc_features(fdom_features);
